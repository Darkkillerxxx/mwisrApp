...
System.IO.Path = function() {
  ...
  this.CheckInvalidPathChars = function(path) {
    var iipc = new RegExp(
      "[" + this.InternalInvalidPathChars.toString().replace(",", "", "g") + "]"
    );
  ...
  this.IsDirectorySeparator = function(c) {
    return (
      c === this.AltDirectorySeparatorChar || c === this.DirectorySeparatorChar
    );
  };
  //---------------------------------------------------------
    ...
    if (!this.CheckInvalidPathChars(path)) {
      ...
      if (length >= 1 && this.IsDirectorySeparator(path.charAt(0))) {
        ...
        if (length >= 2 && this.IsDirectorySeparator(path.charAt(1))) {
          ...
          while (
            num < length &&
            ((path.charAt(num) !== this.DirectorySeparatorChar &&
              path.charAt(num) !== this.AltDirectorySeparatorChar) ||
              --num3 > 0)
          ) {
            num++;
          }
  ...
  this.GetDirectoryName = function(path) {
    ...
    if (path !== null) {
      ...
      if (path.length > rootLength) {
        ...
        while (
          length > rootLength &&
          path.charAt(--length) !== this.DirectorySeparatorChar &&
          path.charAt(length) !== this.AltDirectorySeparatorChar
        ) {
          /* */
        }
    ...
    if (path !== null) {
      if (!this.CheckInvalidPathChars(path)) {
        ...
        while (--startIndex >= 0) {
          ...
          if (
            ch === this.DirectorySeparatorChar ||
            ch === this.AltDirectorySeparatorChar ||
            ch === this.VolumeSeparatorChar
          ) {
            break;
          }
        }
  ...
  this.IsPathRooted = function(path) {
    ...
    if (path !== null) {
      if (!this.CheckInvalidPathChars(path)) {
        ...
        if (
          (length >= 1 &&
            (path.charAt(0) === this.DirectorySeparatorChar ||
              path.charAt(0) === this.AltDirectorySeparatorChar)) ||
          (length >= 2 && path.charAt(1) === this.VolumeSeparatorChar)
        ) {
          return true;
        }
      }
  ...
  this.Combine = function(path1, path2) {
    ...
    if (path1 !== null && path2 !== null) {
      if (
        !(
          this.CheckInvalidPathChars(path1) || this.CheckInvalidPathChars(path2)
        )
      ) {
        if (path2.length === 0) {
          return path1;
