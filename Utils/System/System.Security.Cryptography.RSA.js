...
import Crypto from './System.Security.Cryptography'
...
System.Security.Cryptography.RSACryptoServiceProvider = function() {
  ...
  this.ToXmlString = function(includePrivateParameters) {
    ...
    builder.Append(
      '<Exponent>' + System.Convert.ToBase64String(parameters.Exponent) + '</Exponent>',
    )
    if (includePrivateParameters) {
      ...
      builder.Append(
        '<InverseQ>' + System.Convert.ToBase64String(parameters.InverseQ) + '</InverseQ>',
      )
  ...
  this.ImportCspBlob = function(keyBlob) {
    ...
    if (keyBlob.length !== size) throw new System.Exception('Error: RSA Key BLOB wrong size!')
    //	Public Values
  ...
  this.VerifyHash = function(hash, hashAlgorithmName, signature, fOAEP) {
    ...
    if (hash.length !== decryptedHash.length) return false
    for (var i = 0; i < hash.length; i++) {
      if (hash[i] !== decryptedHash[i]) return false
    }
    return true
  }
  //---------------------------------------------------------
  this.SignData = function(data, hashAlgorithmName, fOAEP) {
    var ha
    if (hashAlgorithmName === 'SHA256') ha = new System.Security.Cryptography.SHA256()
    else ha = new System.Security.Cryptography.SHA1()
