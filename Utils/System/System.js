...
System.Extensions = function() {
  ...
  this.Apply = function() {
    ...
    String.Format = function(format, args) {
      ...
      var value = format.replace(/{(\d+)(:([xX]?\d+))?(,([-]?\d+))?}/g, function(
        matchString,
        number,
      ) {
        ...
        if (hexMatch) {
          ...
          // Change case.
          value = hexMatch[1] === 'x' ? value.toLowerCase() : value.toUpperCase()
          // Add zeros.
          num = parseInt(hexMatch[2])
          var z = ''
          for (var i = value.length; i < num; i++) z += '0'
          value = z + value
        }
        ...
        if (padMatch) {
          ...
          var ln = Math.abs(num)
          var s = ''
          for (i = value.length; i < ln; i++) s += ' '
          value = num >= 0 ? s + value : value + s
        }
        return value
      })
      return value
...
System.Type.Class.Inherit = function() {
  ...
  for (var cid = this.Classes.length - 1; cid > 0; cid--) {
    ...
    Trace.Write(
      '1. Import Class Properties: ' + finObject.Type + '.prototype <- ' + srcObject.Type,
      1,
    )
...
System.Event = function(name) {
  ...
  this.Remove = function(delegate) {
    ...
    /// and a method.
    /// </summary>
    for (var i = this._delegates.length - 1; i >= 0; i = i - 1) {
...
System.EventsManager = function(context) {
  ...
  this.Add = function(node, eventName, eventHandler, capture) {
    ...
    if (node) {
      ...
      if (traceFound)
        Trace.Write(
          'Error: ' + this.GetType().Name + ".Add(...) - node '" + id + "' was not found!",
        )
      success = false
    }
    return success
...
System.Array.Resize = function(array, newSize, defaultValue) {
  ...
  while (newSize > array.length) array.push(defaultValue)
...
System.DateTime.ExpressionUtcDate = new RegExp(
  '([0-9][0-9][0-9][0-9])-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])',
)
System.DateTime.ExpressionUtcTime = new RegExp(
  '([01][0-9]|[2][0123]):([012345][0-9]):([012345][0-9])',
)
...
System.DateTime.ExpressionUtc = new RegExp(
  System.DateTime.ExpressionUtcDate.toString() +
    '[T ]' +
    System.DateTime.ExpressionUtcTime.toString(),
)
...
System.Web.UI.Console = function(id, context) {
  ...
  function createLogStyle() {
    this.LogStyle['onevent'] = {
      F: "'([oO]n)([a-zA-Z]+)'",
      R: '<span style="color: #800000;">\'$1$2\'</span>',
      O: 'g',
    }
    this.LogStyle['on'] = {
      F: '^(on )(.*)',
      R: '<span style="color: #808080;">on </span>$2',
      O: 'i',
    }
    ...
    this.LogStyle['set'] = {
      F: '^(set )(.*)',
      R: '<span style="color: #707000;">set </span>$2',
      O: 'i',
    }
    this.LogStyle['get'] = {
      F: '^(get )(.*)',
      R: '<span style="color: #707000;">get </span>$2',
      O: 'i',
    }
    this.LogStyle['call'] = {
      F: '^(call )(.*)',
      R: '<span style="color: #0000FF;">call</span> $2',
      O: 'i',
    }
    this.LogStyle['warning'] = {
      F: '^(warning)(.*)',
      R: '<span style="color: #b8860b;">Warning</span>$2',
      O: 'i',
    }
    this.LogStyle['error'] = {
      F: '^(error)(.*)',
      R: '<span style="color: #ff0000;">Error</span>$2',
      O: 'i',
    }
    this.LogStyle['exec'] = {
      F: '^(exec)(.*)',
      R: '<span style="color: #cc0099;">exec</span>$2',
      O: 'i',
    }
    this.LogStyle['info'] = { F: '^(info.*)', R: '<span style="color: #c0c000;">$1</span>', O: 'i' }
    this.LogStyle['comment1'] = {
      F: '([^:])//(.*)$',
      R: '$1<span style="color: #008000">//$2</span>',
      O: 'g',
    }
    this.LogStyle['comment2'] = {
      F: '^//(.*)$',
    ...
    for (property in this.LogStyle2) {
      this.LogStyle2[property].Fx = new RegExp(
        this.LogStyle2[property].F,
        this.LogStyle2[property].O,
      )
    }
  }
  //---------------------------------------------------------
    ...
    switch (lcmd) {
      ...
      default:
        if (lcmd.indexOf('/console kbd ') > -1) {
          ...
          sender.Parent.Write(
            "'" + e.Command + "' is not recognized as an internal or external command,",
          )
  ...
  this.ChangeLayout = function(layoutName) {
    if (!System.Type.Class.Exists('System.Web.UI.ShortKeys.KeysManager')) {
      ...
      layoutControl = new System.Web.UI.ShortKeys.KeysManager(
        'LayoutControl',
        this.CmdLine.LineNode,
      )
      if (layoutName !== '') {
      ...
      if (bodyExist) {
        if (!this.Node) {
          ...
          if (!System.Type.Class.Exists('System.Web.UI.HtmlControls.TextBox.CommandLine')) {
            ...
            this.CmdLine = new System.Web.UI.HtmlControls.TextBox.CommandLine(
              cmdTextBox,
              this.Context,
            )
        ...
        if (me.IsWebService) {
          ...
          if (typeof data === 'undefined') {
            ...
            try {
              ...
              Trace.Write(
                'Error: data.setProperty("SelectionLanguage", "XPath") because ' + ex.message,
              )
            }
          }
          //}
...
System.Timers.Timer = function(id, interval, autoReset) {
  ...
  this._action = function() {
    ...
    if (me.TimerId === -1) {
      Trace.Write(
        "Warning: Can't do " + this.id + '[id=' + me.TimerId + ']._action(). Timer is stopped.',
      )
    } else {
  ...
  this.Start = function(autoReset) {
    ...
    if (me.TimerId === -1) {
      ...
      Trace.Write(
        "Warning: Can't " + this.id + '[id=' + me.TimerId + '].Start(). Timer is running.',
      )
    }
  }
  //---------------------------------------------------------
  ...
  this.Resume = function(autoReset) {
    ...
    if (me.TimerId === -1) {
      ...
      Trace.Write(
        "Warning: Can't " + this.id + '[id=' + me.TimerId + '].Resume(). Timer is already running.',
      )
    }
  }
  //---------------------------------------------------------
  ...
  this.Stop = function() {
    ...
    if (me.TimerId === -1) {
      Trace.Write(
        "Warning: Can't " + this.id + '[id=' + me.TimerId + '].Stop(). Timer is already stopped.',
      )
    } else {
...
System.Diagnostics.TraceListener = function(obj) {
  if (
    arguments[0] === System.Diagnostics.TraceInternal ||
    arguments[0].GetType().Name === 'TraceListener'
  )
    return arguments[0]
}
